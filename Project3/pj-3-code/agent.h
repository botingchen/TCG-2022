...
public:
	...
	float UCT(node* cur){				//count for UCT 
		float win_rate = (float) cur->win_game / (float) cur->total_game;
		const float c = sqrt(2);
		float exploitation = sqrt(log(cur->parent->total_game) / cur->total_game);
		float uct = win_rate + c * exploitation;

		return uct; 
	}

	node* selection(){					//select which child node
		node* cur = root;
